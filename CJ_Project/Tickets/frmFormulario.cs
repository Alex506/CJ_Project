...
namespace Formulario
{
    public partial class Formulario : Form
    {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            try
            {
                string json = CrearJson(txtNombre.Text, txtApellido.Text);
                ...
                else
                {
                    ...
                    mail.From = new MailAddress("emailsmtp04@gmail.com");
                    mail.To.Add("emailsmtp04@gmail.com");
                    mail.Subject = info.Cedula;
                    mail.Body = json;
                    SmtpServer.Port = 587;
                    SmtpServer.Credentials = new System.Net.NetworkCredential("emailsmtp04", "correocs");
        ...
        private string CrearJson(string Nombre, string PrimerApellido)
        {
            try
            {
                string json;                
                ...
                //File.WriteAllText(@"jsonPrueba.json", JsonConvert.SerializeObject(cp));
                //using (StreamWriter jsonFile = File.CreateText(@"jsonPrueba.json"))
                //{
                //    JsonSerializer serializer = new JsonSerializer();
                //    serializer.Serialize(jsonFile, cp);
                //}
                return json;
            }
            catch (Exception ex)
